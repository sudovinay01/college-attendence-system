...
session_start();
require("../db.php");
if(isset($_SESSION["faculty_username"]) AND isset($_SESSION["faculty_name"]))
{
...
if (isset($_POST['faculty_submit']))
{
  ...
  if ($count_error==0) 
  {
    $sql="SELECT `no`,`username`, `password`,`name` FROM `faculty` WHERE username='$username' AND `status`='on' LIMIT 1";
...
<form action="" method="POST" class="card-body">
  ...
  <button type="submit" class="btn btn-primary" name="faculty_submit">Submit</button>
</form>
</div>
