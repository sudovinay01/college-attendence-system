...
{
  $sql="UPDATE `faculty` SET `status`='off' WHERE `no`='$no' LIMIT 1";
  if (mysqli_query($db,$sql))
    {
      $_SESSION["msg"]="Faculty <strong>".$_SESSION["rname"]."</strong> is Removed.";
...
<div class="card shadow">
  <div class="card-body">
    <div class="jumbotron">
      <h1>Faculty Remove</h1>
    </div>
     <div class="clearfix">
     <a href="admin_panel_faculty.php"><button type="button" class="btn btn-warning float-left mb-2">Back</button></a>
      </div>
 	<?php
  if (!isset($_GET["no"]) OR !preg_match('/^[0-9]+$/', $no)) 
{
   header('location:admin_panel_faculty.php');
}
else
{
  	$sql="SELECT `no`,`id`,`name`,`branch` FROM `faculty` where `no`='$no' AND `status`='on'";
