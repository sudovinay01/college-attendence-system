...
<div class="flex card shadow ">
  <div class="card-body ">
        <div class="jumbotron">
            <h1>Add Students(csv)</h1>
        </div>
        <a href="students_list.php"><button class="btn btn-info">Done</button></a>
        
<?php 
$filename=$_FILES["file"]["name"];
if (move_uploaded_file($_FILES['file']['tmp_name'], __DIR__.'./students_data/'. $_FILES["file"]['name'])) {
...
if($ext==".csv")
{
    ...
    <p><span class="text-danger">*</span>Fields should be - Id,Name,Username,Password.</p>
    ...
    <tbody id="myTable">';
  $file = fopen('./students_data/'.$filename, "r");
         while (($emapData = fgetcsv($file, 10000, ",")) !== FALSE)
         {
          $passwordx=$emapData[3];
            if (!isset($emapData[0]) OR !isset($emapData[1]) OR !isset($emapData[2]) OR !isset($emapData[3])) 
            {
                echo "<tr><td colspan='6' class='bg-danger text-white'>Invalid data given.Please follow given rules.</td></tr>";
            }
            else
            {
                $sql="SELECT `username` FROM `".$_SESSION["abranch"]."-".$_SESSION["ayear"]."-".$_SESSION['asection']."` WHERE `username`='".$emapData[2]."' LIMIT 1";
                $result=mysqli_query($db,$sql);
                $row = mysqli_fetch_array($result);
                $sql2="SELECT `id` FROM `".$_SESSION["abranch"]."-".$_SESSION["ayear"]."-".$_SESSION['asection']."` WHERE `id`='".$emapData[0]."' LIMIT 1";
                $result=mysqli_query($db,$sql2);
                 $row2 = mysqli_fetch_array($result);
                if ($row>0)
                    {  
                         $sql="SELECT `name`,`username` FROM `".$_SESSION["abranch"]."-".$_SESSION["ayear"]."-".$_SESSION['asection']."` WHERE `username`='".$emapData[2]."' AND `status`='off' LIMIT 1";
                     $result=mysqli_query($db,$sql);
                     $row = mysqli_fetch_array($result);
                 if ($row>0)
                 ...
                 {
               echo'<tr>';
                    if(!preg_match('/^[\w]+$/', $emapData[0]) OR !preg_match('/^[\w ]+$/', $emapData[1]) OR !preg_match('/^[\w]+$/', $emapData[2]) OR !preg_match('/^[\w]+$/', $emapData[3]))
                       {
                     ...
                     $sql = "INSERT INTO `".$_SESSION["abranch"]."-".$_SESSION["ayear"]."-".$_SESSION['asection']."`(`id`,`name`, `username`, `password`) VALUES ('$emapData[0]','$emapData[1]','$emapData[2]','$emapData[3]')";
...
{
    unlink('./students_data/'.$filename) or die("Couldn't delete file");
