...
<div class="card shadow">
  <div class="card-body">
    <div class="jumbotron">
     ...
     <a href="add_faculty.php"><button type="button" class="btn btn-primary mb-2">Add Faculty</button></a>
     <br/>
    <form method="POST" action="import_csv.php" class="border border-primary pb-2 rounded bg-light text-dark" enctype="multipart/form-data" >
        
          <div class="custom-file m-2">
            <p>add faculty bulk</p>
    <label>upload csv (for bulk upload)</label><br/>
    Fields should be - Id,Name,Username,Password,Branch.<br/>branch-cse/civil/ece/has<br/>
    ...
    $sql="SELECT count(`id`) as total FROM `faculty` where `status`='on'";
    $result=mysqli_query($db,$sql);
    $row = mysqli_fetch_assoc($result);
    if($row['total']>0)
    {
      $sql="SELECT `no`,`id`,`name`,`branch` FROM `faculty` where `status`='on'";
      $result=mysqli_query($db,$sql);
    ...
    while($row = mysqli_fetch_array($result))
    {
     ...
     echo '<td><a href="faculty_view.php?no='.$row['no'].'"><span class="btn btn-primary" >View</span></a></td>
    <td><a href="faculty_edit.php?no='.$row['no'].'"><span class="btn btn-warning" >Edit</span></a></td><td><a href="faculty_remove.php?no='.$row['no'].'"><span class="btn btn-danger" >Remove</span></a></td>
      </tr>';
    }
   ...
   {
    echo '<div class="alert alert-warning"><strong>No data of faculty.</strong></div>';
    }
    echo "</tbody>";
    ?>
