...
{
   $sql="UPDATE `".$_SESSION["table"]."` SET `status`='on' WHERE `username`='".$_SESSION['un']."' AND `status`='off' LIMIT 1";
...
{
  if(!preg_match('/^[\w]+$/', $_POST['id']) OR !preg_match('/^[\w ]+$/', $_POST['name']) OR !preg_match('/^[\w]+$/', $_POST['password']))
  {
  ...
  if (isset($_SESSION['un'])) 
  {
      $sql="UPDATE `".$_SESSION["table"]."` SET `id`='".$id."',`name`='".$name."',`password`='".$password."',`status`='on' WHERE `username`='".$_SESSION['un']."' AND `status`='off' LIMIT 1";
        if (mysqli_query($db,$sql))
    {
      if ( $_SESSION["table"]=='faculty')
      {
        $_SESSION["msg"]="Faculty <strong>".$_POST['name']."</strong> is added.";
      unset($_SESSION['un']);
      header('location:admin_panel_faculty.php');
      }
      else
...
<div class="flex card shadow ">
  ...
  <form action="" method="POST" class="container ">
    <div class="container bg-success text-white p-3 rounded">
      <h2>1)Restoring</h2>
      <p>Are you trying to restore account with username - <?php echo $_SESSION['un'].' and  Name - '.$_SESSION['unn'];?></p>*Use this if u have accidentally removed this account  
      </p>
    ...
    <form action="" method="POST" class="container ">
      ...
      <h2>2)New account</h2>
      <p>Are you trying to create new account with username - <?php echo $_SESSION['un'];?></p>if yes type id,name & password
      </p>
    </div>
